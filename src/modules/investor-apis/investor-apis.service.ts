...
import { PortfolioSummaryDto, SchemeSummaryDto } from './dto/portfolio-summary.dto';
...
export class InvestorApisService {
    ...
    async getPortfolioSummary(investorId: string): Promise<PortfolioSummaryDto> {
        ...
        for (const [prodcode, trxns] of schemeMap.entries()) {
            ...
            if (summary && summary.units_held > 0.001 && summary.invested_amount > 0) { // Only active holdings
    ...
    private async calculateSchemeMetrics(prodcode: string, trxns: any[]): Promise<SchemeSummaryDto | null> {
        const getIsinForScheme = async (code: string, origin: 'CAMS' | 'KARVY'): Promise<string> => {
            if (!code) return '';
            if (origin === 'CAMS') {
                const schemeDetail = await this.schemeRepo.findOne({
                    where: { id: Raw(() => `CONCAT(amc_code, sch_code) = :code`, { code }) },
                    select: ['isin_no']
                });
            ...
            if (first.origin === 'CAMS') {
                ...
                const camsSip = await this.camsSipRepo.findOne({
                    where: {
                        ...
                        scheme_code: first.prodcode ? (first.prodcode.startsWith(first.amc_code) ? first.prodcode.slice(first.amc_code.length) : first.prodcode) : undefined,
                    },
                });
                if (camsSip) {
                    is_sip = true;
        ...
        trxns.forEach(t => {
            ...
            let nature: string = '';
            if (t.origin === 'CAMS') {
                date = t.traddate ? new Date(t.traddate) : (t.postdate ? new Date(t.postdate) : new Date());
                amount = Number(t.amount);
                units = Number(t.units);
                nature = (t.trxn_nature || '').toUpperCase();
            } else if (t.origin === 'KARVY') {
                date = t.transaction_date ? new Date(t.transaction_date) : (t.process_date ? new Date(t.process_date) : new Date());
                amount = Number(t.amount);
                units = Number(t.units);
                nature = (t.transaction_description || t.transaction_head || '').toUpperCase();
            } else {
                return;
            }
            ...
            const type = ((t.trxntype || t.transaction_type || '') as string).toUpperCase();
            const isOutflow =
        ...
        const navs = isinNo
            ? await this.navRepo.find({
                where: [
                    { isinPayoutGrowth: isinNo },
                    { isinReinvestment: isinNo }
                ],
                order: { navDate: 'DESC' },
                take: 1
            })
