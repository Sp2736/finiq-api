import { Controller, Get, UseGuards, Request, UnauthorizedException, HttpCode, HttpStatus, Param } from '@nestjs/common';
...
export class InvestorApisController {
    ...
    async getKpis(@Request() req) {
        if (!req.user || req.user.type !== 'investor') {
            throw new UnauthorizedException('Only authenticated investors can access this endpoint');
        }

        const investorId = req.user.id;
        const holdingsReport = await this.holdingsService.getHoldingsReport(investorId);
        ...
        const result = {
            ...
            unrealised_gain: (holdingsReport.unrealised_gains_st || 0) + (holdingsReport.unrealised_gains_lt || 0),
            abs_return_percent: holdingsReport.abs_percent || 0,
            xirr_percent: holdingsReport.xirr_percent || 0
        };

        return ResponseFormatter.success(result, 'Portfolio KPIs retrieved successfully');
    }
    ...
    async getPortfolioGraph(@Request() req) {
        ...
        return ResponseFormatter.success(result, 'Portfolio graph data retrieved successfully');
    }
    ...
    async getPortfolioSummary(@Request() req) {
        ...
        if (req.user.type !== 'investor') {
            throw new UnauthorizedException('Only investors can access this endpoint');
        }

        const investorId = req.user.id;
        const result = await this.service.getPortfolioSummary(investorId);
        return ResponseFormatter.success(result, 'Portfolio summary retrieved successfully');
    }
}
