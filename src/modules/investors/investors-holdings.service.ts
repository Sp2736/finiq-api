...
export class InvestorsHoldingsService {
  ...
  // -----------------------------------------------------------------------
  private async fetchAndCacheCams(
    investorId: string,
    cacheKey: string,
  ): Promise<any[]> {
  ...
  // -----------------------------------------------------------------------
  private async fetchAndCacheKarvy(
    investorId: string,
    cacheKey: string,
  ): Promise<any[]> {
    const rows = await this.investorRepo.query(
  ...
  // -----------------------------------------------------------------------
  private async fetchPortfolioRows(
    arrInvestorId: string[],
  ): Promise<PortfolioRow[]> {
    const [camsRows, karvyRows] =
      arrInvestorId.length === 1
  ...
  private processPortfolioRows(portfolioRows: PortfolioRow[]) {
    ...
    for (const row of portfolioRows) {
      ...
      const navDate = row.current_nav_date
        ? new Date(row.current_nav_date)
        : new Date();
      ...
      const costOfRedeemed = Math.max(
        0,
        redemptionSwpStp - (realisedGainsSt + realisedGainsLt),
      );
      const totalDeployedCapital = activeInvestedCapital + costOfRedeemed;
      const absReturn =
        totalDeployedCapital > 0 ? (netPnl / totalDeployedCapital) * 100 : 0;
      const firstPurchaseDate = row.first_purchase_date
      ...
      const transactions = (row.transactions ?? []).map((tx: any) => ({
        ...
        unrealised_gains:
          currentNav * parseFloat((tx.units ?? 0).toFixed(4)) -
      ...
      const xirrFlows: { amount: number; date: Date }[] = transactions
        .filter(
          (tx) =>
            tx.amount !== 0 &&
            !NON_CASHFLOW_TYPES.has(
              (tx.transaction_type || '').toUpperCase().trim(),
            ),
        )
        .map((tx) => {
          ...
          return {
            amount: isOutflow ? tx.amount : -tx.amount,
            date: tx.transaction_date,
          };
        });

      if (dividendPayout > 0)
        xirrFlows.push({ amount: dividendPayout, date: navDate });
      if (currentValue > 0)
        xirrFlows.push({ amount: currentValue, date: navDate });
      ...
      if (realisedGainsSt !== 0 || realisedGainsLt !== 0) {
        allTaxTransactions.push({
          transaction_date: navDate,
          realised_gains_st: realisedGainsSt,
          realised_gains_lt: realisedGainsLt,
        });
      }
    ...
    for (const key of Object.keys(aggregate) as (keyof typeof aggregate)[]) {
      (aggregate as any)[key] = parseFloat(
        (aggregate[key] as number).toFixed(2),
      );
    }
  ...
  async getHoldingsReport(investorId: string) {
    ...
    const portfolioRows: PortfolioRow[] = await this.fetchPortfolioRows([
      investorId,
    ]);
    ...
    const { funds, aggregate, allXirrFlows, allTaxTransactions } =
      this.processPortfolioRows(portfolioRows);

    // -- Capital gains tax (realised) ------------------------------------
    const { realised_tax_stcg, realised_tax_ltcg, ltcgByFy } =
      this.taxCalculator.calculateRealisedTax(allTaxTransactions, taxRules);

    const { estimated_unrealised_tax_stcg, estimated_unrealised_tax_ltcg } =
    ...
    funds.forEach((fund) => {
      ...
      overallYesterdayValue +=
        (fund.current_value || 0) - (fund.todays_pnl || 0);
      overallNetPnl += fund.net_pnl || 0;
    });
    const overallTodaysPnlPercent =
      overallYesterdayValue > 0
        ? (overallTodaysPnl / overallYesterdayValue) * 100
        : 0;
    ...
    const overallAbsReturn =
      overallTotalDeployed > 0
        ? (overallNetPnl / overallTotalDeployed) * 100
        : 0;
  ...
  async getPortfolioGraph(investorId: string) {
    ...
    const unifiedTransactions = [
      ...
      (a, b) => new Date(a.traddate).getTime() - new Date(b.traddate).getTime(),
    );

    if (!unifiedTransactions.length) return [];

    const isinCache = new Map<string, string>();
    const getIsinCached = async (
      prodcode: string,
      origin: string,
    ): Promise<string> => {
    ...
    let txIndex = 0;
    const activeLotsByScheme = new Map<
      string,
      { origin: string; lots: { units: number; cost: number }[] }
    >();
    let counter = 1;

    while (curDate <= endDate || dataPoints.length === 0) {
      while (
        txIndex < unifiedTransactions.length &&
        new Date(unifiedTransactions[txIndex].traddate) <= curDate
      ) {
        ...
        const isOutflow =
          ...
          Number(tx.amount) < 0;
        const isDividend =
          type.includes('DIVIDEND') && nature.includes('PAYOUT');

        if (!isDividend && !isOutflow) {
          slot.lots.push({
            units: Math.abs(Number(tx.units)),
            cost: Math.abs(Number(tx.amount)),
          });
        } else if (isOutflow) {
          ...
          while (toSell > 0.001 && slot.lots.length) {
            ...
            if (lot.units <= toSell) {
              ...
              lot.cost -= lot.cost * (toSell / lot.units);
              lot.units -= toSell;
              toSell = 0;
            }
          }
        ...
        if (unitsHeld > 0.001) {
          ...
          if (isinNo) {
            const navInfo = await this.navRepo
              ...
              .where(
                '(n.isinPayoutGrowth = :isin OR n.isinReinvestment = :isin)',
                { isin: isinNo },
              )
  ...
  // Transaction Report
  // -----------------------------------------------------------------------
  async getTransactionReport(investorId: string) {
    const query = `
SELECT
    i.investor_name,
    i.mobile_no,
    i.email,
    cit.traddate                    AS transaction_date,
    cit.trxn_type_flag              AS transaction_type,
    csd.sch_name             		AS fund_description,
    cit.folio_no                    AS folio_number,
    csd.isin_no                     AS isin,
    cit.amount,
    cit.purprice                    AS nav,
    cit.units,
    (t.nav * cit.units)             AS current_amount,
    'CAMS'                          AS rta
FROM cams_investor_transactions cit
LEFT JOIN cams_investor_static_details cisd
    ON cisd.foliochk = cit.folio_no
    AND cisd.product = cit.prodcode
INNER JOIN investors i
    ON i.id = cisd.investor_id
LEFT JOIN cams_scheme_details csd
    ON CONCAT(csd.amc_code, csd.sch_code) = cit.prodcode
LEFT JOIN latest_nav t
    ON (t.isin_payout_growth = csd.isin_no OR t.isin_reinvestment = csd.isin_no)
WHERE cisd.investor_id = $1

UNION ALL

-- Karvy Transactions (deduplicated via subquery)
SELECT
    investor_name,
    mobile_no,
    email,
    transaction_date,
    transaction_description         AS transaction_type,
    fund_description,
    folio_number,
    isin,
    amount,
...
FROM (
    SELECT
        ...
        i.email,
        kt.transaction_date,
        kt.transaction_description,
        kimd.fund_description,
        kt.folio_number,
        kt.isin,
        kt.amount,
        kt.nav,
        kt.units,
        (t.nav * kt.units)          AS current_amount,
        ROW_NUMBER() OVER (
            PARTITION BY kt.transaction_id
            ORDER BY kt.transaction_date DESC
        )                           AS rn
    FROM karvy_transactions kt
    LEFT JOIN karvy_investor_master_data kimd
        ON kimd.folio = kt.folio_number
        AND kimd.product_code = kt.product_code
    INNER JOIN investors i
        ON i.id = kimd.investor_id
    LEFT JOIN karvy_scheme_details ksd
        ON ksd.product_code = kt.product_code
    LEFT JOIN latest_nav t
        ON (t.isin_payout_growth = kt.isin OR t.isin_reinvestment = kt.isin)
    WHERE i.id = $1
        AND kt.transaction_flag != 'R'
) karvy_deduped
WHERE rn = 1

ORDER BY transaction_date desc;
        `;
    const rows = await this.investorRepo.query(query, [investorId]);
    ...
    const transactions = rows.map((row) => ({
      ...
      nav: row.nav ? parseFloat(row.nav) : 0,
      units: row.units ? parseFloat(row.units) : 0,
      current_amount: row.current_amount ? parseFloat(row.current_amount) : 0,
      rta: row.rta,
    }));

    return {
      investor_name: firstRow.investor_name,
      mobile_no: firstRow.mobile_no,
