import { Controller, Get, Post, Query, Param, UseGuards, Req, BadRequestException } from '@nestjs/common';
...
export class BrokerageDistributionController {
    ...
    async processAll() {
        ...
        return { message: 'Brokerage distribution processing initiated successfully' };
    }
    ...
    async getSubBrokerAmc(
        ...
        if (!fromDate || !toDate) {
            throw new BadRequestException(ErrorMessages[ApiErrorCode.QUERY_PARAMS_REQUIRED]);
        }
        const companyId = req.user?.roles?.find((r: any) => r.company_id)?.company_id || req.user?.company_id;
        if (!companyId) {
            throw new BadRequestException(ErrorMessages[ApiErrorCode.COMPANY_ID_NOT_FOUND]);
        }
        return this.distributionService.getSubBrokerAmcAggregation(companyId, fromDate, toDate);
    }
    ...
    async getDetailedSummary(
        ...
        const companyId = req.user?.roles?.find((r: any) => r.company_id)?.company_id || req.user?.company_id;
        return this.distributionService.getDetailedBrokerageDistribution(companyId, fromDate, toDate, groupBy);
    }
}
