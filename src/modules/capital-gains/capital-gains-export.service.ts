...
export class CapitalGainsExportService {
  ...
  private async generateExcel(data: any, from_date:string, to_date:string): Promise<Buffer> {
    ...
    const createSheet = (
      ...
      applyStyle(r1, 1, r1, 15, {
        font: { bold: true, size: 14, color: { argb: THEME.navy } },
      ...
      applyStyle(r2, 1, r2, 15, {
        font: { italic: true, size: 10, color: { argb: THEME.textMuted } },
        alignment: { horizontal: 'right' },
      });
      ...
      applyStyle(r5, 1, r5, 13, {
        fill: {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: THEME.offWhite },
        },
        font: { size: 9, color: { argb: THEME.textPrimary } },
        alignment: { horizontal: 'left' },
        border: { left: borderAll.left },
      });
      applyStyle(r5, 14, r5, 15, {
        fill: {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: THEME.offWhite },
      ...
      applyStyle(r6, 1, r6, 13, {
        ...
        font: { size: 9, color: { argb: THEME.textPrimary } },
        alignment: { horizontal: 'left' },
        border: { bottom: borderAll.bottom, left: borderAll.left },
      });
      applyStyle(r6, 14, r6, 15, {
        fill: {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: THEME.offWhite },
        ...
        font: { size: 9, color: { argb: THEME.textPrimary } },
        alignment: { horizontal: 'right' },
        border: { bottom: borderAll.bottom, right: borderAll.right },
      });
      ...
      if (funds.length > 0) {
        ...
        applyStyle(rMF, 1, rMF, 15, {
          fill: {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: THEME.navy },
          },
          font: { bold: true, color: { argb: THEME.white }, size: 12 },
          alignment: { horizontal: 'left' },
        });
        ws.addRow([]);
        ...
        funds.forEach((mf: any) => {
          ...
          applyStyle(rTitle, 1, rTitle, 15, {
            fill: {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: THEME.steelLight },
            },
            font: { bold: true, color: { argb: THEME.navy } },
            border: borderAll,
          });
          ...
          applyStyle(rSuper, 8, rSuper, 12, {
            fill: {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: THEME.navyMid },
            },
            font: { bold: true, color: { argb: THEME.white } },
            alignment: { horizontal: 'center', vertical: 'middle' },
            border: borderAll,
          });
          applyStyle(rSuper, 13, rSuper, 13, {
            fill: {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: THEME.navy },
            },
            font: { bold: true, color: { argb: THEME.white }, size: 9 },
            alignment: {
              horizontal: 'center',
              vertical: 'middle',
              wrapText: true,
          ...
          applyStyle(rSuper, 14, rSuper, 15, {
            fill: {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: THEME.navyMid },
          ...
          applyStyle(rSub, 1, rSub, 15, {
            fill: {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: THEME.steel },
            },
            font: { bold: true, color: { argb: THEME.white }, size: 9 },
            alignment: {
              horizontal: 'center',
          ...
          mf.transactions.forEach((tx: any, idx: number) => {
            ...
            for (let c = 1; c <= 15; c++) {
              ...
              ws.getCell(rTx, c).alignment = {
                horizontal: align,
                vertical: 'middle',
              };

              if ([4, 5, 6, 7, 10, 11, 12, 13, 14, 15].includes(c))
                ws.getCell(rTx, c).numFmt = '#,##0.00';
            }

            if (Number(tx.shortTermPL) > 0)
              ws.getCell(rTx, 14).font = {
                color: { argb: THEME.positive },
                bold: true,
              };
            else if (Number(tx.shortTermPL) < 0)
              ws.getCell(rTx, 14).font = {
                color: { argb: THEME.negative },
                bold: true,
              };

            if (Number(tx.longTermPL) > 0)
              ws.getCell(rTx, 15).font = {
                color: { argb: THEME.positive },
                bold: true,
              };
            else if (Number(tx.longTermPL) < 0)
              ws.getCell(rTx, 15).font = {
                color: { argb: THEME.negative },
                bold: true,
              };
          ...
          for (let c = 6; c <= 15; c++) {
            ...
            if ([6, 11, 14, 15].includes(c))
              ws.getCell(rTot, c).numFmt = '#,##0.00';
          }
          ws.addRow([]);
        });
      }
      ...
      if (isEquity && data.capitalGainSummary) {
        const rEq1 = ws.addRow([
          'Mutual Funds — Period-Wise Capital Gain Summary',
        ]).number;
        ws.mergeCells(rEq1, 1, rEq1, 9);
        applyStyle(rEq1, 1, rEq1, 9, {
          fill: {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: THEME.navy },
      ...
      applyStyle(rDisc, 1, rDisc, 15, {
        font: { bold: true, size: 12, color: { argb: THEME.negative } },
        alignment: { wrapText: true, vertical: 'top' },
      });
    };

    const equityFunds = data.mutualFunds.filter((mf: any) =>
    ...
    const debtFunds = data.mutualFunds.filter((mf: any) =>
      String(mf.assetClass).toLowerCase().includes('debt'),
    );
  ...
  private generatePDF(data: any, from_date:string, to_date:string): Buffer {
    ...
    const drawInvestorBand = () => {
      ...
      doc.text(`Email: ${inv.email}`, PW - MR - 4, Y + 10.5, {
        align: 'right',
      });
      ...
      rows.push([
        {
          ...
          styles: {
            ...
            fillColor: C.steelLight,
            textColor: C.navy,
          },
        },
        {
          content: fmt(totalSell),
          styles: {
            fontStyle: 'bold',
            halign: 'right' as const,
            fillColor: C.steelLight,
        ...
        { content: '', styles: { fillColor: C.steelLight } },
        {
          ...
          styles: {
            ...
            halign: 'right' as const,
            fillColor: C.steelLight,
            textColor: totalST >= 0 ? C.positive : C.negative,
          },
        },
        {
          content: fmt(totalLT),
          styles: {
            ...
            halign: 'right' as const,
            fillColor: C.steelLight,
            textColor: totalLT >= 0 ? C.positive : C.negative,
          },
        },
      ]);
      ...
      autoTable(doc, {
        ...
        head: [
          [
            ...
            {
              content: 'Corresponding Subscription',
              colSpan: 5,
              styles: {
                halign: 'center',
                fillColor: C.navyMid,
      ...
      autoTable(doc, {
        ...
        head: [
          ...
          [
            {
              content: 'Buy Value',
              styles: { fillColor: C.navyMid, textColor: C.white },
            },
            {
              content: 'Sell Value',
              styles: { fillColor: C.navyMid, textColor: C.white },
            ...
            {
              content: 'Buy Value',
              styles: { fillColor: C.steel, textColor: C.white },
            },
            {
              content: 'Sell Value',
              styles: { fillColor: C.steel, textColor: C.white },
      ...
      autoTable(doc, {
        ...
        head: [
          [
            ...
            {
              content: 'Short Term — Quarter-wise',
              colSpan: 5,
              styles: {
                halign: 'center',
                fillColor: C.navyMid,
            ...
            {
              content: 'Long Term — Quarter-wise',
              colSpan: 5,
              styles: {
                halign: 'center',
                fillColor: C.steel,
