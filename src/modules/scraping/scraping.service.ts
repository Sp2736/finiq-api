...
export class ScrapingService {
    ...
    private async readCaptcha(page: Page): Promise<string> {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            ...
            this.ensureDebugDir();
            fs.writeFileSync(
                path.join(DEBUG_DIR, `captcha_crop_attempt${attempt}_${Date.now()}.png`),
                captchaBuffer
            );
        ...
        throw new Error('Could not OCR captcha correctly after 3 attempts. Check debug_screenshots/.');
    }
    ...
    private async refreshAndReadCaptcha(page: Page): Promise<string> {
        ...
        try {
            ...
            try {
                ...
                this.logger.warn('Could not click captcha refresh — reading current captcha');
            }
        }
    ...
    async login(payload: LoginPayload): Promise<LoginResult> {
        ...
        try {
            ...
            const screenshotPath = await this.screenshot(page, `03_after_next_${nextStep}`);
    ...
    //  BULL QUEUE
    // ─────────────────────────────────────────────
    async queueLoginJob(payload: LoginPayload): Promise<{ jobId: string | number }> {
        ...
        this.logger.log(`Queued login job: ${job.id} for user: ${payload.username}`);
        return { jobId: job.id };
    }
