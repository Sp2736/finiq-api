...
export class UserManagementController {
    ...
    async createUser(@Req() req: any, @Body() dto: CreateUserDto) {
        if (!dto.company_id) {
            dto.company_id = req.user?.roles?.find((r: any) => r.company_id)?.company_id || req.user?.company_id;
        }
    ...
    async getSubBroker(@Param('id') id: string) {
        ...
        return ResponseFormatter.success(result, 'Sub-broker details fetched successfully');
    }

    @Put(':id')
    @Roles(UserRole.COMPANY_ADMIN)
    async updateSubBroker(@Param('id') id: string, @Body() dto: UpdateSubBrokerDto) {
