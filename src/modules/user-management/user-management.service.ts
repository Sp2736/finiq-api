...
export class UserManagementService {
  ...
  private async createSubBroker(dto: CreateUserDto) {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (parentId) {
        const parent = await manager.findOne(SubBroker, {
          where: { id: parentId },
        });
  ...
  private async createInvestor(dto: CreateUserDto) {
    ...
    const parentBroker = await this.subBrokerRepo.findOne({
      where: { id: parentId },
    });
    if (!parentBroker) {
  ...
  private async isDescendantOf(
    ...
    while (currentId) {
      ...
      const parent = await this.subBrokerRepo.findOne({
        where: { id: currentId },
      });
  ...
  private async rebuildDescendantPaths(manager: any, brokerId: string) {
    const broker = await manager.findOne(SubBroker, {
      where: { id: brokerId },
    });
    if (!broker) return;

    const children = await manager.find(SubBroker, {
      where: { parent_id: brokerId },
    });
    for (const child of children) {
