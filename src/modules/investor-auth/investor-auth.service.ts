...
import { SendInvestorOtpDto, LoginInvestorDto, VerifyInvestorOtpDto, ResetInvestorPasswordDto } from './dto/investor-auth.dto';
...
export class InvestorAuthService {
    ...
    async login(dto: LoginInvestorDto) {
        ...
        if (!investor.password_hash) {
            throw new UnauthorizedException('Please ask your distributor to generate credentials first');
        }
    ...
    async verifyOtpForPasswordReset(dto: VerifyInvestorOtpDto) {
        ...
        if (!otpLog || otpLog.otp_code !== otp || otpLog.is_used || new Date(otpLog.expires_at) < new Date()) {
