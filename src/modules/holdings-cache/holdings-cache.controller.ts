...
export class HoldingsCacheController {
  ...
  async refreshCache() {
    ...
    return ResponseFormatter.success(
      null,
      'Holdings cache refresh triggered in background',
    );
  }
  ...
  async getCompanySummary(@Request() req: any) {
    ...
    const result =
      await this.holdingsCacheService.getSummaryByCompany(companyId);
    return ResponseFormatter.success(
      result,
      'Company holdings summary retrieved successfully',
    );
  }
  ...
  async getTopContributors(@Request() req: any) {
    const companyId = this.getCompanyId(req.user);
    const result =
      await this.holdingsCacheService.getTopContributingClients(companyId);
    return ResponseFormatter.success(
      result,
      'Top contributing clients retrieved successfully',
    );
  }
  ...
  async getInvestorsList(
    ...
    const { page: pageNum, limit: limitNum } =
      PaginationHelper.getPaginationParams(page, limit, maxLimit ?? 5000);
    const { data, total } =
      await this.holdingsCacheService.getCompanyInvestorsList(
        companyId,
        pageNum ?? 1,
        limitNum ?? 10,
        search,
  ...
  async getInvestorHoldings(
    @Request() req: any,
    @Body('investor_id') investorId: string,
  ) {
    if (!investorId) {
      throw new BadRequestException(
        'investor_id is required in the request body',
      );
    }
    const companyId = this.getCompanyId(req.user);
    try {
      const result = await this.holdingsCacheService.getInvestorHoldings(
        companyId,
        investorId,
      );
      return ResponseFormatter.success(
      ...
      throw new ForbiddenException(
        logAndSanitize(
          new Logger('HoldingsCacheController'),
          'Error fetching investor holdings',
          error,
          'You do not have permission to view these holdings.',
        ),
      );
    }
  }
