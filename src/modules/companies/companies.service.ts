...
export class CompaniesService extends BaseService<Company, any, any> {
    ...
    async create(createCompanyDto: CreateCompanyDto) {
        ...
        try {
            ...
            if (!tenantId) {
                ...
                if (!defaultTenant) {
                    throw new BadRequestException('No active tenant found to associate with the company.');
                }
            ...
            let user = await this.userRepo.findOne({ where: { phone_number: createCompanyDto.phone_number } });
            if (!user) {
                user = new User();
    ...
    async createCompanyUser(userPayload: any, dto: CreateCompanyUserDto) {
        ...
        const adminRole = userPayload.roles.find(r => r.role === UserRole.COMPANY_ADMIN);

        if (!adminRole || !adminRole.company_id) {
            throw new BadRequestException('User is not a company admin or missing company context');
        }
        ...
        try {
            ...
            let user = await this.userRepo.findOne({ where: { phone_number: dto.phone_number } });
            ...
            await queryRunner.rollbackTransaction();
            this.logger.error(`Error creating company user: ${error.message}`, error.stack);
            throw error;
        } finally {
            await queryRunner.release();
        }
    ...
    async createArn(userPayload: any, dto: CreateCompanyArnDto) {
        const adminRole = userPayload.roles.find((r: any) => r.role === UserRole.COMPANY_ADMIN);
        ...
        const company = await this.dataSource.manager.findOne(Company, { where: { id: companyId } });
        if (!company) {
