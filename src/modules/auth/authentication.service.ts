...
export class AuthenticationService {
    ...
    async sendOtp(dto: SendOtpDto) {
        ...
        const otpExpiryMinutes = this.configService.get<number>('OTP_EXPIRATION_MINUTES') || 5;

        // Generate OTP
        const otp = crypto.randomInt(Math.pow(10, otpLength - 1), Math.pow(10, otpLength) - 1).toString();
    ...
    async verifyOtp(dto: VerifyOtpDto) {
        ...
        if (otpRecord.is_used) {
            throw new BadRequestException(ErrorMessages[ApiErrorCode.OTP_ALREADY_USED]);
        }
        ...
        if (!user) {
            throw new UnauthorizedException(ErrorMessages[ApiErrorCode.USER_NOT_FOUND]);
        }
    ...
    private async generateAuthResponse(user: User, profiles: UserProfile[]) {
        ...
        const accessTokenExpiry = this.configService.get<string>('JWT_ACCESS_TOKEN_EXPIRATION') || '24h';
        const refreshExpiryDays = this.configService.get<number>('JWT_REFRESH_TOKEN_EXPIRATION_DAYS') || 7;

        // Generate tokens
        const access_token = this.jwtService.sign(payload as any, { expiresIn: accessTokenExpiry as any });
        const refresh_token = crypto.randomBytes(64).toString('hex');
        const refreshExpiresAt = new Date(Date.now() + refreshExpiryDays * 24 * 60 * 60 * 1000);

        await this.repository.saveRefreshToken(user.id, refresh_token, refreshExpiresAt);
